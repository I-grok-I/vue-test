...
export default function useTelegram(): useTelegramComposableState {
   ...
   *
   * @param reference The ref to store the button
  ...
  function showMainButton(text: string, callback: () => void): void {
    prepareDebugButton(debugMainButton, 'fake-main-button') // render only if in browser, will not work in tg client

    if (mainButtonCallback.value !== null) {
      WebApp.MainButton.offClick(mainButtonCallback.value) // delete the old callback if exists
    }

    mainButtonCallback.value = callback

    WebApp.MainButton.text = text ?? 'Подтвердить'
    WebApp.MainButton.onClick(mainButtonCallback.value)
    WebApp.MainButton.isVisible = true

    // if debugging in browser, set the main fake button's inner text to the text which was passed as the 'text' argument
    if (debugMainButton.value !== undefined) {
      debugMainButton.value.innerText = text ?? 'Подтвердить'
